...
GITHUB_USER="${GITHUB_USER:-kkkhhello}"
GITHUB_REPO="${GITHUB_REPO:-V2bX}"
GITHUB_BRANCH="${GITHUB_BRANCH:-main}"
FIXED_VERSION="${FIXED_VERSION:-v0.4.0}"
RAW_BASE="https://raw.githubusercontent.com/${GITHUB_USER}/${GITHUB_REPO}/${GITHUB_BRANCH}"
RELEASE_BASE="https://github.com/${GITHUB_USER}/${GITHUB_REPO}/releases/download"
...
download_V2bX_archive() {
    local version=$1
    local asset="V2bX-linux-${arch}.zip"
    local raw_url="${RAW_BASE}/release-assets/${version}/${asset}"
    local release_url="${RELEASE_BASE}/${version}/${asset}"

    echo -e "开始安装 V2bX ${version}"
    echo -e "下载地址：${raw_url}"
    wget --no-check-certificate -N --progress=bar -O /usr/local/V2bX/V2bX-linux.zip "${raw_url}"
    if [[ $? -eq 0 ]]; then
        return 0
    fi

    echo -e "${yellow}从仓库文件下载失败，尝试从 GitHub Release 下载：${release_url}${plain}"
    wget --no-check-certificate -N --progress=bar -O /usr/local/V2bX/V2bX-linux.zip "${release_url}"
    return $?
}
...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        last_version="${FIXED_VERSION}"
    else
        last_version=$1
    fi

    download_V2bX_archive "${last_version}"
    if [[ $? -ne 0 ]]; then
        echo -e "${red}下载 V2bX ${last_version} 失败，请确认 ${GITHUB_USER}/${GITHUB_REPO} 已上传 release-assets/${last_version}/V2bX-linux-${arch}.zip 或创建同名 GitHub Release 资产${plain}"
        exit 1
    fi
...
EOF
    ...
    curl -fLo /usr/bin/V2bX -Ls "${RAW_BASE}/V2bX.sh"
    if [[ $? -ne 0 ]]; then
        echo -e "${red}下载 V2bX 管理脚本失败，请确认 ${RAW_BASE}/V2bX.sh 可以访问${plain}"
        exit 1
    fi
    ...
    if [[ $first_install == true ]]; then
        ...
        if [[ $if_generate == [Yy] ]]; then
            curl -fLo ./initconfig.sh -Ls "${RAW_BASE}/initconfig.sh"
            if [[ $? -ne 0 ]]; then
                echo -e "${red}下载配置生成脚本失败，请确认 ${RAW_BASE}/initconfig.sh 可以访问${plain}"
                exit 1
            fi
