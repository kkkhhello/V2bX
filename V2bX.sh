...
install() {
    bash <(curl -fLs "${RAW_BASE}/install.sh") "${FIXED_VERSION}"
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    if [[ $# == 0 ]]; then
        echo && echo -n -e "输入指定版本(默认${FIXED_VERSION}): " && read version
    else
        version=$2
    fi
    if [[ -z "${version}" ]]; then
    ...
    bash <(curl -fLs "${RAW_BASE}/install.sh") "${version}"
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate "${RAW_BASE}/V2bX.sh"
    if [[ $? != 0 ]]; then
        echo ""
